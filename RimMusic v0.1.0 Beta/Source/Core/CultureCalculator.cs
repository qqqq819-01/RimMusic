...
namespace RimMusic.Core
{
    ...
    public static class CultureCalculator
    {
        ...
        /// <summary>
        /// Flattens the instrument roster, processing user-defined mutes, overrides, and expansions.
        /// </summary>
        public static List<string> GetFlattenedInstruments(CulturalRoster roster)
        {
            ...
            string hashKey = GetHashForSourceId(roster.sourceId);
            var cacheData = CultureOracleCache.GetData(hashKey);
            var profile = cacheData?.OverrideProfile;

            for (int i = 0; i < roster.activeInstruments.Count; i++)
            {
                if (profile != null && profile.MutedSlots.Count > i && profile.MutedSlots[i]) continue;

                string inst = roster.activeInstruments[i];
                if (profile != null && profile.CustomSlots.Count > i && !string.IsNullOrEmpty(profile.CustomSlots[i]))
                {
                    inst = profile.CustomSlots[i];
                }
            ...
            if (profile != null && profile.ExtraInstruments != null)
            {
                foreach (string exInst in profile.ExtraInstruments)
                {
                    if (!string.IsNullOrWhiteSpace(exInst)) finalInsts.Add(exInst);
                }
            }
