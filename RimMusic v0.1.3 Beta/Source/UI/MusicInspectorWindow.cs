...
namespace RimMusic.UI
{
    ...
    public class MusicHUDWindow : Window
    {
        ...
        public override void PreOpen()
        {
            ...
            RefreshTelemetry();
            // 注入模式(装了 Music Manager)下不走自播 playlist
            if (!RuntimeSongRegistrar.IsMusicManagerInstalled) RealtimeMusicEngine.InitializePlaylist();
            else RuntimeSongRegistrar.RegisterAllFromDisk(); // 顺手扫描补注册新文件
        }
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            try
            {
                ...
                l.Label("RimMusic_HUD_AutoGen".Translate(AutoGenCoordinator.GetCooldownDisplayString()));
                GUI.color = Color.white;
            }
            catch { }
            ...
            // 注入模式: 装了 Music Manager → 隐藏播放/切歌按钮,仅留滚动标题 + 生成功能
            // 替补模式: 未装 Music Manager → 照搬 Music Manager 风格的上一/暂停/下一播放控制
            bool fallbackPlayer = !RuntimeSongRegistrar.IsMusicManagerInstalled;
            ...
            if (showUI && fallbackPlayer)
            {
                // 替补播放控制(仅在未装 Music Manager 时显示)
            ...
            // 滚动标题:装了 Music Manager 时占满,未装时给按钮留位
            float titleX = (showUI && fallbackPlayer) ? playerArea.x + 125f : playerArea.x + 10f;
            float titleW = (showUI && fallbackPlayer) ? playerArea.width - 130f : playerArea.width - 20f;
            Rect titleRect = new Rect(titleX, playerArea.y + 8f, titleW, 24f);

            // 滚动标题:注入模式显示托管提示;替补模式显示当前曲目名
            string trackName = fallbackPlayer
                ? "RimMusic_TrackLabel".Translate(RealtimeMusicEngine.CurrentTrackName)
                : "RimMusic_HUD_InjectedMode".Translate();
        ...
        private async void GeneratePromptSilent()
        {
            ...
            try
            {
                // 整体 deadline 90 秒:推演提示词超时则断开,避免卡死流程
                var promptTask = new MusicAIClient().GenerateParsedMusicPromptAsync(ctx);
                var completed = await Task.WhenAny(promptTask, Task.Delay(TimeSpan.FromSeconds(90)));
                if (completed != promptTask)
                {
                    _statusText = "RimMusic_Status_Timeout".Translate();
                    Messages.Message("RimMusic_Error_PromptTimeout".Translate(), MessageTypeDefOf.RejectInput, false);
                    return;
                }
                _lastGeneratedPrompt = await promptTask;
                _statusText = "RimMusic_Status_Generated".Translate();
            }
        ...
        private async void GenerateAndPlayRealMusic()
        {
            ...
            // 按所选后端检凭据:不无脑检 302ai key,选了哪个生成源就判哪个的 token/key 没填
            int mode = RimMusicMod.Settings?.AudioBackendMode ?? 0;
            RimMusic.Core.Backends.IAudioBackend backend = mode == 0 ? new RimMusic.Core.Backends.Backend302Ai()
                : mode == 1 ? new RimMusic.Core.Backends.BackendMiniMaxWeb()
                : mode == 2 ? (RimMusic.Core.Backends.IAudioBackend)new RimMusic.Core.Backends.BackendFlowMusicWeb()
                : null;
            if (backend == null || !backend.IsConfigured)
            {
                Messages.Message("RimMusic_Error_ApiKeyEmpty".Translate(backend?.Name ?? "?"), MessageTypeDefOf.RejectInput, false);
                return;
            }

            _isGeneratingRealMusic = true;
            // 左上角提示带后端源名:按 AudioBackendMode 拿对应后端标签翻译
            string backendLabel = "RimMusic_Engine_Backend_302".Translate();
            try
            {
                backendLabel = (mode == 0 ? "RimMusic_Engine_Backend_302" : mode == 1 ? "RimMusic_Engine_Backend_MiniMax" : "RimMusic_Engine_Backend_FlowMusic").Translate();
            }
            catch { }
            Messages.Message("RimMusic_Msg_Requesting".Translate(backendLabel), MessageTypeDefOf.NeutralEvent, false);

            try
            {
                MusicContext ctx = MusicContext.Build(MusicGameComponent.ActiveProtagonist, true);

                // 推演提示词阶段:整体 90 秒超时(沿用 GeneratePromptSilent 的短超时)
                var promptTask = new MusicAIClient().GenerateParsedMusicPromptAsync(ctx);
                var completed = await Task.WhenAny(promptTask, Task.Delay(TimeSpan.FromSeconds(90)));
                if (completed != promptTask)
                {
                ...
                string finalPrompt = await promptTask;
                _lastGeneratedPrompt = finalPrompt;
                _statusText = "RimMusic_Status_Sent".Translate();

                // 生成音乐阶段:整体 8 分钟超时(生成耗时长,给足时间但避免永久卡死)
                var genTask = RealtimeMusicEngine.RequestAndPlayMusic(finalPrompt, ctx.focus_name);
                var genCompleted = await Task.WhenAny(genTask, Task.Delay(TimeSpan.FromMinutes(8)));
                if (genCompleted != genTask)
                {
                    _statusText = "RimMusic_Status_Timeout".Translate();
                    Messages.Message("RimMusic_Error_GenTimeout".Translate(), MessageTypeDefOf.RejectInput, false);
                    return;
                }
                await genTask;
            }
