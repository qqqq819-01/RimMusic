    ...
    public class MusicContext
    {
        ...
        public static MusicContext Build(Pawn forcedFocus = null, bool isRealGeneration = false)
        {
            ...
            if (Current.ProgramState == ProgramState.Playing && Find.CurrentMap != null)
            {
                ...
                float threatRatio = CultureCalculator.GetThreatRatio(map);
                float stealThreshold = RimMusicMod.Settings?.ThreatStealThreshold ?? 0.3f;
                float highTempoThreshold = RimMusicMod.Settings?.ThreatHighTempoThreshold ?? 0.75f;
                float dominateThreshold = RimMusicMod.Settings?.ThreatDominateThreshold ?? 0.90f;

                // 四档紧张度:
                //   < stealThreshold        → Safe(和平, BPM 低, 文化乐器主导)
                //   ≥ stealThreshold        → Danger(战歌, BPM 80-115, 文化乐器保留+紧张感)
                //   ≥ highTempoThreshold    → High Tempo(高BPM战歌, BPM ~150, 文化乐器仍保留仅紧张度提升)
                //   ≥ dominateThreshold     → High Danger(末日, BPM 130+, 完全接管 Heavy/EDM, 丢弃文化乐器)
                if (threatRatio >= dominateThreshold) ctx.danger = "High Danger";
                else if (threatRatio >= highTempoThreshold) ctx.danger = "High Tempo";
                else if (threatRatio >= stealThreshold) ctx.danger = "Danger";
                else ctx.danger = "Safe";
            ...
            // [FIXED] Force capture of ANY selected pawn, including animals, bypassing protagonist filters.
            Pawn narrativePawn = forcedFocus ?? (Find.Selector.SingleSelectedThing as Pawn);
            bool isRaid = ctx.danger != "Safe";
            bool isMacro = MusicGameComponent.IsMacroMode && narrativePawn == null;

            if (narrativePawn != null)
            {
                // Native HostileTo correctly checks for MentalStateDefOf.Manhunter
                bool isEnemy = narrativePawn.HostileTo(Faction.OfPlayer);

                // [FIXED] Specific state labels for Animals
                if (narrativePawn.RaceProps != null && narrativePawn.RaceProps.Animal)
                    ctx.state = isEnemy ? "Micro (Manhunter Animal)" : "Micro (Wild/Domestic Animal)";
                else if (narrativePawn.IsPrisoner)
                    ctx.state = "Micro (Prisoner Target)";
                else if (isEnemy)
                    ctx.state = "Micro (Enemy Target)";
                else
                    ctx.state = isRaid ? "Combat (Individual)" : "Micro (Selected)";
            }
            else
            {
                if (isMacro)
            ...
            if (narrativePawn != null)
            {
                ctx.focus_name = narrativePawn.Name != null ? narrativePawn.LabelShort : narrativePawn.LabelCap;
                ctx.activity = GetActivity(narrativePawn);

                ctx.mood_level = narrativePawn.needs?.mood != null ? narrativePawn.needs.mood.CurLevel.ToString("P0") : "Unknown";
                ctx.thoughts = GetTopThoughts(narrativePawn);

                bool isEnemy = narrativePawn.HostileTo(Faction.OfPlayer);
                if (isRaid && !isEnemy && (!narrativePawn.RaceProps?.Animal ?? true))
                {
            ...
            {
                string dominantVibe = null;

                // 乐器抢夺需要 pawn 作为文化依据。narrativePawn==null(没点击小人/自动生成)时,
                // 按同 vibe 兜底逻辑挑一个:袭击挑敌方 pawn,和平时挑殖民者。
                // 不兜底的话 CalculateCulturalInstruments 拿 null 算不出文化乐器,AI 只能生成纯通用氛围。
                Pawn instrumentsPawn = narrativePawn;
                if (instrumentsPawn == null && Find.CurrentMap.mapPawns != null)
                {
                    instrumentsPawn = isRaid
                        ? Find.CurrentMap.mapPawns.AllPawnsSpawned.FirstOrDefault(p => p.HostileTo(Faction.OfPlayer) && !p.Downed)
                ...
                {
                    ctx.culture_instruments = CultureCalculator.CalculateCulturalInstruments(Find.CurrentMap, isRaid, instrumentsPawn, out dominantVibe);
                }
                else
                {
                    ctx.culture_instruments = CultureCalculator.GetStaticBaseInstruments(instrumentsPawn);
                }

                var musicComp = Current.Game.GetComponent<CulturalMusicComponent>();
                if (musicComp != null)
                {
                    if (!string.IsNullOrEmpty(dominantVibe))
                    {
                        ctx.culture_vibe = dominantVibe;
                    }
                    else
                    {
        ...
        private static string BuildFullContext(Pawn p, MusicContext ctx)
        {
            ...
            string fullName = p.Name != null ? p.Name.ToStringFull : p.LabelCap;
            string genderStr = p.gender != Gender.None ? p.gender.GetLabel() : "Genderless";
            string ageStr = p.ageTracker != null ? p.ageTracker.AgeBiologicalYears.ToString() : "Unknown";

            modules.Add($"[P1]\n{fullName} ({genderStr}, {ageStr})");
            ...
            else if (p.HostileTo(Faction.OfPlayer)) roleLabel = "Enemy";
            ...
            string moodDisplay = p.needs?.mood != null ? p.needs.mood.CurLevel.ToString("P0") : "N/A";
            idSb.AppendLine($"Mood: {moodDisplay} | Memory: {ctx.thoughts}");
